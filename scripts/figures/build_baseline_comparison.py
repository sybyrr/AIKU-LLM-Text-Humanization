...
SOURCE = ROOT / "results/reports/humanizer_skill_baseline_n100_20260909.html"
...
(METRICS / f"{STEM}.json").write_text(json.dumps(data, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
...
plt.close(fig)
print(json.dumps({"rows": data_rows, "exports": [str(OUT / f"{STEM}.{ext}") for ext in ("png", "svg")] + [str(METRICS / f"{STEM}.json")]}, indent=2))
