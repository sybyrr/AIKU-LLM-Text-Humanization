"""Plot detector adaptation and G1/G2 performance from adversarial/RESULTS.md."""
import json
import os
...
SOURCE = ROOT / "adversarial/RESULTS.md"
...
internal = table("| Detector | G1 ASR | G2 ASR | G2−G1 |")
external = table("| 변형 | 평균 AI작성률 | AI 판정률 | ASR |")
n = int(re.search(r"동일 test ID ([\d,]+)개", source)[1].replace(",", ""))
rows = []
for source_label, detector, role in [
    ("D0", "D0", "Frozen"), ("D1", "D1", "Adapted"),
]:
    row, = [r for r in internal if r["Detector"] == source_label]
...
data = {
    ...
    "source_sections": ["D1 학습 결과", "G1과 G2 비교", "CopyKiller 결과"],
...
(METRICS / "adversarial_results.json").write_text(json.dumps(data, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
