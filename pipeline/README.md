...
파이프라인의 진입점이다. 실제 데이터 처리·학습 구현은 `../scripts/`에 있고 여기서는 설정,
단계 연결, 재개, 산출물 검증을 담당한다.
...
도메인마다 달라지는 것은 `human_pool.jsonl`을 만드는 Stage 1a와 설정값뿐이다. 데이터,
생성물, 모델, 로그는 공개 저장소에 넣지 않는다.
...
`workspace` 아래에 `dataset/`, `models/`, `logs/`가 생긴다. 저장소 안에 비공개 데이터를
둘 경우에는 `--workspace .`을 사용한다. 각 생성기 URL의 llama-server는 실행 전에 별도로
기동해야 한다.
...
필수 필드는 `doc_id`와 본문 하나(`text`, `body`, `human_text`)다.
...
그 이름을 지정한다. 대화 도메인은 `n_turn`을 넣고 `gate.dialogue=true`로 둔다.

### 생성기

`generators` 배열의 항목마다 서로 다른 OpenAI-compatible chat endpoint를 지정한다. 생성은
항목 순서대로 수행되며 각 결과는 `name=JSONL` arm으로 gate에 모두 전달된다. Qwen3 계열은
`no_think=true`가 필요하다.
...
기본 실행은 완료 산출물을 건너뛴다. 범위를 제한하려면 `--from-stage`와 `--to-stage`를 쓴다.
`--force`는 완료 판정을 무시하고 하위 단계를 다시 호출하지만, `generate.py`와 hard-negative
채굴처럼 자체 재개 기능이 있는 구현은 기존 성공분을 재사용한다. 완전 초기화는 산출물을 별도로
백업한 뒤 명시적으로 치워야 한다. 단계별 stdout/stderr와 상태는
`logs/{domain}/pipeline/`에 기록된다.
...
`hard_negative.no_repeat_ngram_size`의 기본값 3은 기존 채굴 조건을 재현한다. 최종 plain
평가는 반복방지를 사용하지 않으므로 두 분포가 다르다. 붕괴 출력도 rejected 후보로 노출하는
실험을 하려면 이 값을 0으로 명시하되, 기존 결과와 같은 실험이라고 취급하면 안 된다.

## 평가 출력

`evaluate.py`는 test split에서 `human`, `x_ai`, `SFT`, `DPO` 네 행을 문서별로 저장한다.
표본 평가에서는 generator 비율을 largest-remainder 방식으로 보존하고 seed 42로 섞는다.
`evaluation.sample_size=0`은 test 전량을 뜻한다.

각 JSONL 행에는 다음이 들어간다.
...
모집단별 raw ASR·collapse·cosine·반복 진단 요약을 저장한다.
평가기는 같은 실행에서 생성한 human 점수의 95분위를 임계로 사용한다. 서로 다른 target
detector의 ASR 절댓값을 직접 비교하기보다 같은 target 안에서 SFT→DPO를 비교한다.

## CPU 검증

GPU 없이 설정, 단계 연결, 층화 표본, 붕괴 판정을 검사할 수 있다.
