...
사용:
  python adversarial/check_env.py                                # KCI loop, 활성 GPU 환경
  python adversarial/check_env.py --profile team-news --team-root .
  python adversarial/check_env.py --cpu-only --profile none      # 로컬 코드 스모크 전
종료코드 0 = 전부 통과.
"""
import argparse
import json
...
def main():
    ...
    print()
    print("전부 통과 — 진행 가능" if ok_all else "실패 항목 있음 — adversarial/requirements.txt 주석과 notes/31 § 서버 확인 참조")
