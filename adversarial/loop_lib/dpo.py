...
초기 KCI 기본값: lr 5e-6, β=0.1, 5 epoch. y_w 는 arm 별로 다르다 (인간앵커/자기앵커).
완료한 뉴스 team_news 설정: β=2, 1 epoch, token 평균 log-prob와 DPOP(λ=5).
DPOP는 chosen log-prob가 reference보다 낮아질 때 margin에서 hinge penalty를 뺀다.
"""
import torch
